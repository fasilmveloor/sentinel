...
from ..models import AttackType, Endpoint, HttpMethod, ScanTask, Severity, Vulnerability
...
class BenchmarkRunner:
    ...
    def __init__(self, ground_truth: GroundTruthDatabase | None = None):
    ...
    async def run_benchmark(
        ...
        [v for v in known_vulns if not v.is_true_positive]
    ...
    async def _get_endpoints(self, target: BenchmarkTarget, base_url: str, spec_path: str | None) -> list[Endpoint]:
        if spec_path:
    ...
    async def _run_scan(self, endpoints: list[Endpoint], base_url: str, attack_types: list[AttackType], auth_token: str | None, timeout: int, verbose: bool) -> list[Vulnerability]:
        try:
...
async def run_crapi_benchmark(base_url: str = "http://localhost:8888", spec_path: str | None = None, verbose: bool = False) -> BenchmarkResult:
    runner = BenchmarkRunner()
    return await runner.run_benchmark(target=BenchmarkTarget.CRAPI, base_url=base_url, spec_path=spec_path, verbose=verbose)

async def run_juice_shop_benchmark(base_url: str = "http://localhost:3000", spec_path: str | None = None, verbose: bool = False) -> BenchmarkResult:
    runner = BenchmarkRunner()
...
async def run_owasp_benchmark(base_url: str = "http://localhost:8080", spec_path: str | None = None, verbose: bool = False) -> BenchmarkResult:
    runner = BenchmarkRunner()
...
async def run_dvwa_benchmark(base_url: str = "http://localhost:8081", spec_path: str | None = None, verbose: bool = False) -> BenchmarkResult:
    runner = BenchmarkRunner()
    return await runner.run_benchmark(target=BenchmarkTarget.DVWA, base_url=base_url, spec_path=spec_path, verbose=verbose)

async def run_webgoat_benchmark(base_url: str = "http://localhost:8082", spec_path: str | None = None, verbose: bool = False) -> BenchmarkResult:
    runner = BenchmarkRunner()
    return await runner.run_benchmark(target=BenchmarkTarget.WEBGOAT, base_url=base_url, spec_path=spec_path, verbose=verbose)

async def run_all_benchmarks(targets: dict[BenchmarkTarget, str] | None = None, verbose: bool = False) -> list[BenchmarkResult]:
    if targets is None:
        targets = {
