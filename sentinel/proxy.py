...
from .passive import PassiveFinding, PassiveFindingType, create_passive_scanner
...
class ProxyTrafficStore:
    ...
    def get_flow(self, flow_id: str) -> TrafficFlow | None:
        ...
        for pattern, title in patterns:
            if re.search(pattern, body, re.IGNORECASE):
                findings.append(PassiveFinding(
                    ...
                    title=title,
                    description="Sensitive authentication data found in response body",
                    evidence=pattern,
                    location="body",
                    remediation="Remove sensitive data from response or encrypt it"
                ))

    def detect_api_endpoint(self, flow: TrafficFlow) -> str | None:
...
class SentinelProxy:
    ...
    def extract_openapi_spec(self) -> dict:
        ...
        for _endpoint_key, data in endpoints.items():
            path = data["path"]
            method = data["method"]
